require_relative 'ecrpp2'

module NArrayMethod

  def bianry(meth, ope=nil)
    ope = meth if ope.nil?
...
module NArrayType
  ...
  def type_var
    @opts[:type_var] ||= "lattice_c" + class_name
  end

  def math_var
    @opts[:math_var] ||= "lattice_m" + class_name + "Math"
  end
  ...
  def real_type_var
    @opts[:real_type_var] ||= "lattice_c" + real_class_name
  end
  ...
  def upcast(c=nil, t=nil)
    ...
    if c
      ...
      @opts[:upcast] << "cr_hash_aset(hCast, lattice_c#{c}, #{t});"
    else
      @opts[:upcast]
    end
  end

  def upcast_cr(c, t=nil)
    @opts[:upcast] ||= [] of T
    if t
      t = "lattice_c#{t}"
    else
    ...
    if c == "Integer"
      @opts[:upcast] << "#ifdef CRYSTAL_INTEGER_UNIFICATION"
      @opts[:upcast] << "cr_hash_aset(hCast, cr_cInteger, #{t});"
      @opts[:upcast] << "#else"
      @opts[:upcast] << "cr_hash_aset(hCast, cr_cFixnum, #{t});"
      @opts[:upcast] << "cr_hash_aset(hCast, cr_cBignum, #{t});"
      @opts[:upcast] << "#endif"
    else
      @opts[:upcast] << "cr_hash_aset(hCast, cr_c#{c}, #{t});"
    end
  end
end
...
class StoreNum < Store
  def condition(klass)
    "IS_INTEGER_CLASS(#{klass}) || #{klass} == cr_cFloat || #{klass} == cr_cComplex"
  end
end
